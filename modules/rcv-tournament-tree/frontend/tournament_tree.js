...
function displayData (data) {
  dataDiv.forEach(element => {
    ...
    blueTeam.querySelector('.tt_tag').innerHTML = match?.teams.blueTeam.tag ?? ""
    blueTeam.querySelector('.tt_name').innerHTML = match?.teams.blueTeam.name ?? ""
    resizeText(blueTeam.querySelector('.tt_name'))
    if (match?.teams.blueTeam.tag)
    blueTeam.querySelector('.tt_score').innerHTML = match?.teams.blueTeam.score ?? 0

    redTeam.querySelector('.tt_tag').innerHTML = match?.teams.redTeam.tag ?? ""
    redTeam.querySelector('.tt_name').innerHTML = match?.teams.redTeam.name ?? ""
    resizeText(redTeam.querySelector('.tt_name'))
    if (match?.teams.redTeam.tag)
    redTeam.querySelector('.tt_score').innerHTML = match?.teams.redTeam.score ?? 0
  });
}
